...
"""A tuple representing an activation function and its derivative."""

loss_function_type = tuple[Callable[[np.ndarray, np.ndarray], float], Callable[[np.ndarray, np.ndarray], np.ndarray]]
"""A tuple representing a loss function and its derivative."""
...
def forward_softmax(x: np.ndarray) -> np.ndarray:
    """The forward pass of the softmax activation function."""
    shifted = x - np.max(x, axis=1, keepdims=True)
    exps    = np.exp(shifted)
    return exps / np.sum(exps, axis=1, keepdims=True)

def backward_softmax(x: np.ndarray, delta: np.ndarray) -> np.ndarray:
    """The backward pass of the softmax activation function."""
    y = forward_softmax(x)
...
def fixed_learning_rate() -> learning_rate_optimizer_type:
    """Returns a fixed learning rate optimizer."""
...
def time_based_decay(func: Callable[[int], float]) -> learning_rate_optimizer_type:
    """Returns a time-based decay learning rate optimizer."""
